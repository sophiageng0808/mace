...
# -----------------------------
# Softcap models (ABSOLUTE PATHS)
# -----------------------------
SOFTCAP_MODELS = [
    ("softcap_log_radial", "/h/400/sophiageng/mace_worktrees/jobs_softcap/360319_3_softcap_log_radial/softcap_log_radial.model"),
    ("softcap_quadratic", "/h/400/sophiageng/mace_worktrees/jobs_softcap/360319_2_softcap_quadratic/softcap_quadratic.model"),
    ("softcap_inv_softplus", "/h/400/sophiageng/mace_worktrees/jobs_softcap/360319_1_softcap_inv_softplus/softcap_inv_softplus.model"),
    # ("softcap_none", "/h/400/sophiageng/mace_worktrees/jobs_softcap/360319_0_softcap_none/softcap_none.model"),
]
...
SUMMARY_HEADER = [
    ...
    "total_nan_count", "total_inf_count",
]
...
def run_scan(
    ...
    for d in dist:
        ...
        nan_count += int(np.isnan(E))
        inf_count += int(np.isinf(E))
...
if __name__ == "__main__":
    ...
    softcap_root = Path(__file__).resolve().parent
    os.chdir(softcap_root)

    run_id = args.run_name or datetime.now().strftime("%Y%m%d_%H%M%S")
    group_name = "softcap_new"
    ...
    ALL_CSV_PATH = group_dir / "all_curves_metrics.csv"
    FAILED_CSV_PATH = group_dir / "failed_curves_metrics.csv"
    SUMMARY_CSV_PATH = group_dir / "model_summary_metrics.csv"

    ensure_csv(ALL_CSV_PATH, CSV_HEADER)
    ensure_csv(FAILED_CSV_PATH, CSV_HEADER)
    ensure_csv(SUMMARY_CSV_PATH, SUMMARY_HEADER)

    calc_map = {}
    for model_name, model_path in SOFTCAP_MODELS:
        mp = Path(model_path)
        if not mp.exists():
    ...
    for idx in range(n_to_run):
        ...
        for i in range(nat):
            ...
            for model_name, calc in calc_map.items():
                ...
                acc["nan_count"] += int(metrics["nan_count"])
                acc["inf_count"] += int(metrics["inf_count"])
    ...
    for model_name, acc in per_model_acc.items():
        ...
        row = [
            ...
            int(acc["nan_count"]), int(acc["inf_count"]),
        ]
        write_csv(SUMMARY_CSV_PATH, row)

    if use_wandb:
        art = wandb.Artifact(f"scan_outputs_overfit100_{group_name}_{run_id}", type="results")
        art.add_file(str(ALL_CSV_PATH))
        art.add_file(str(FAILED_CSV_PATH))
        art.add_file(str(SUMMARY_CSV_PATH))
        wandb.run.log_artifact(art)

    print(f"[DONE] Group {group_name} finished.")
    print(f"  All-curves CSV:  {ALL_CSV_PATH}")
    print(f"  Failed-only CSV: {FAILED_CSV_PATH}")
    print(f"  Summary CSV:     {SUMMARY_CSV_PATH}\n")

    global_summary_csv = OUTPUT_ROOT / "all_groups_model_summary.csv"
    pd.DataFrame([{
        "run_id": run_id,
        "group": group_name,
        "model": name,
    ...
    print("[DONE] Finished softcap group.")
