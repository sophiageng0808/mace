...
class MACE(torch.nn.Module):
    def __init__(
        ...
        self.radial_embedding = RadialEmbeddingBlock(
            ...
            softcap_kind=softcap_kind,
            softcap_eps=softcap_eps,
            softcap_concat=softcap_concat,
            softcap_apply_env=softcap_apply_env,
            # -------------------------------------------------------------
        )
...
class AtomicDipolesMACE(torch.nn.Module):
    def __init__(
        ...
        edge_irreps: Optional[o3.Irreps] = None,  # pylint: disable=unuse
        softcap_kind: str = "none",  # pylint: disable=unused-argument
        softcap_eps: float = 1e-3,  # pylint: disable=unused-argument
        softcap_concat: bool = True,  # pylint: disable=unused-argument
        softcap_apply_env: bool = True,  # pylint: disable=unused-argument
        # ---------------------------------------------------------------------
    ):
        super().__init__()
        self.register_buffer(
