...
def main():
    ...
    parser.add_argument("--data_extxyz", type=str, default=str(DATA_EXTXYZ))
    ...
    for _, row in df.iterrows():
        ...
        start_xyz_path = geom_dir / f"{mol_label}_{safe_pair}_start.xyz"
        end_xyz_path = geom_dir / f"{mol_label}_{safe_pair}_end.xyz"
        ...
        ase_write(str(start_xyz_path), atoms_start, format="xyz")
        ase_write(str(end_xyz_path), atoms_end, format="xyz")
        print(f"[OK] Wrote {start_xyz_path}")
        print(f"[OK] Wrote {end_xyz_path}")
